...
def getAngleMask(b, orientations, angle):
    ...
    const = (
        (2 ** (2 * order)) * (np.math.factorial(order) ** 2) / (orientations * np.math.factorial(2 * order))
    )  # Scaling constant
    angle = np.mod(np.pi + angle - np.pi * (b) / orientations, 2 * np.pi) - np.pi  # Mask angle mask
    anglemask = np.multiply(
        2 * np.sqrt(const) * np.cos(angle) ** order, (np.abs(angle) < np.pi / 2)
    )  # Make falloff smooth
    return anglemask
...
def getFiltersSmoothWindow(dims, orientations, cosOrder=6, filtersPerOctave=6, complex=True, height=None):
    ...
    const = (
        (2 ** (2 * cosOrder))
        * (np.math.factorial(cosOrder) ** 2)
        / ((cosOrder + 1) * np.math.factorial(2 * cosOrder))
    )
    for k in range(int(filts), 0, -1):
        shift = np.pi / (cosOrder + 1) * k + 2 * np.pi / 7
        radFilters.append(
            np.sqrt(const) * np.multiply(np.power(np.cos(rad - shift), cosOrder), windowFnc(rad, shift))
        )
    ...
    totalCrop = total[
        center[0] - lodims[0] : center[0] + lodims[0] + 1, center[1] - lodims[1] : center[1] + lodims[1] + 1
    ]
    lopass = np.zeros(dims)
    lopass[
        center[0] - lodims[0] : center[0] + lodims[0] + 1, center[1] - lodims[1] : center[1] + lodims[1] + 1
    ] = np.abs(np.sqrt(1 - totalCrop))
...
def FIRWindowBP(delta, fl, fh):
    ...
    for k in range(1, batches + 1):
        ...
        freqDom = np.multiply(
            freqDom, np.tile(transferFunction, [freqDom.shape[0], freqDom.shape[1]]).reshape(freqDom.shape)
        )
