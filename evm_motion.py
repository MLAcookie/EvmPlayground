...
def get_phase_amplified_frames(
    ...
    pyrtype="octave",
    sigma=0,
    temporalFilter=fil.FIRWindowBP,
    scalevideo=1,
    frames=None,
):
    ...
    term.Terminal.Clear()
    term.Terminal.Println("Preparing capture")
    ...
    term.Terminal.Println(f"Loading: {filename}")

    vid = np.zeros([h, w, nC, nF], frame.dtype)
    vid[:, :, :, 0] = frame
    for k in range(1, nF):
        term.Terminal.Println(f"{k}/{nF-1}")
    ...
    ht = fil.maxSCFpyrHt(np.zeros((h, w)))
    if pyrtype == "octave":
        filters = fil.getFilters((h, w), np.power(2, np.arange(0, -ht - 1, -1).astype(float)), 4)
        print("Using octave bandwidth pyramid\n")
    elif pyrtype == "halfOctave":
        filters = fil.getFilters(
            (h, w), np.power(2, np.arange(0, -ht - 0.5, -0.5).astype(float)), 8, twidth=0.75
        )
        print("Using half octave bandwidth pyramid\n")
    elif pyrtype == "smoothHalfOctave":
        filters = fil.getFiltersSmoothWindow((h, w), 8, filtersPerOctave=2)
        print("Using half octave pyramid with smooth window.\n")
    elif pyrtype == "quarterOctave":
        filters = fil.getFiltersSmoothWindow((h, w), 8, filtersPerOctave=4)
        print("Using quarter octave pyramid.\n")
    else:
        raise ValueError("Invalid Filter Types")

    term.Terminal.Println(f"Pyrtype: {pyrtype}")

    croppedFilters, filtIDX = fil.getFilterIDX(filters)
    ...
    def buildLevel(im_dft, k):
        return np.fft.ifft2(
            np.fft.ifftshift(np.multiply(croppedFilters[k][0], im_dft[filtIDX[k, 0], filtIDX[k, 1]]))
        )
    ...
    term.Terminal.Println("Moving video to Fourier domain")
    ...
    for level in range(1, numLevels - 1):
        ...
        term.Terminal.Println(f"Processing level {level} of {numLevels}")
        ...
        for frameIDX in range(0, nF):
            ...
            if sigma != 0:
                phaseOfFrame = fil.AmplitudeWeightedBlur(phaseOfFrame, np.abs(originalLevel) + eps, sigma)
    ...
    for frameIDX in range(0, nF):
        try:
            lowpassFrame = np.multiply(
                vidFFT[filtIDX[level, 0], filtIDX[level, 1], frameIDX], np.power(croppedFilters[-1][0], 2)
            )
            magnifiedLumaFFT[filtIDX[level, 0], filtIDX[level, 1], frameIDX] = (
...
def phase_amplify_to_file(
    ...
    outname = "{0}-{1}-band{2:0.2f}-{3:0.2f}-alpha{4}.mp4".format(
        writetag,
